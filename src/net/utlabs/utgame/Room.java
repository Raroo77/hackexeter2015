...
public class Room {
    ...
    public static final int VECTOR_FIELD_RESOLUTION = 8;
    ...
    public void render(int delta, Player player) {
        ...
        GL11.glColor4f(1, 0, 1, 0.7F);
        GL11.glBegin(GL11.GL_LINES);
        for (int i = 0; i < mField.length; i++)
            for (int j = 0; j < mField[i].length; j++) {
                float px = vectorFieldToPixel(i + .5f);
                float py =  vectorFieldToPixel(j + .5f);
                Vector v = mField[i][j].multiply(100, new Vector());
                GL11.glVertex2f(px, py);
                GL11.glVertex2f(px + v.mX, py + v.mY);
            }
        GL11.glEnd();
    ...
    public int vectorFieldToPixel(float fieldCoord) {
        return (int) (fieldCoord / VECTOR_FIELD_RESOLUTION * 60);
    }
